...
public class JSCronDateCalculator implements JSCronVisitor<List<Long>> {
	...
	public List<Long> calculate(List<JSStatement> statementList, long start, long end, int size) {
		...
		for (JSStatement statement : statementList) {
			set.addAll(visitStatement(statement, start, end, size));
		}
	...
	public List<Long> visitScheduledJobStatement(JSCronScheduledJobStatement statement, long start, long end, int size) {
		if (statement.expression != null) {
			return visitExpression(statement.expression, start, end, size, 0);
		}
	...
	public List<Long> visitScheduleExpression(JSCronScheduleExpression expression, long start, long end, int size) {
		...
		if (expression.yearExpression != null) {
			...
			List<Long> longList = visitExpression(expression.yearExpression);
		...
		Set<Integer> monthSet = toIntegerSet(visitExpression(expression.monthExpression));
		...
		while (!done) {
			...
			if (expression.dayOfMonthExpression instanceof JSCronAllExpression) {
				dateSet.addAll(visitExpression(expression.dayOfWeekExpression, start, end, size, months));
			} else if (expression.dayOfWeekExpression instanceof JSCronAllExpression) {
				dateSet.addAll(visitExpression(expression.dayOfMonthExpression, start, end, size, months));
			} else {
				dateSet.addAll(visitExpression(expression.dayOfMonthExpression, start, end, size, months));
				dateSet.addAll(visitExpression(expression.dayOfWeekExpression, start, end, size, months));
			}
	...
	@Override
	public List<Long> visitListExpression(JSCronListExpression expression, long start, long end, int size, int months) {
		List<Long> list = new ArrayList<>();
		for (JSExpression item : expression.expressionList) {
			list.addAll(visitExpression(item, start, end, size, months));
		}
		return list;
	}
	...
	public List<Long> visitDayExpression(JSCronDayExpression expression, long start, long end, int size, int months) {
		List<Long> list = visitExpression(expression.expression, start, end, size, months);
	...
	@Override
	public List<Long> visitStatement(JSStatement statement) {
		return visitStatement(statement, 0, Long.MAX_VALUE, 0);
	}

	@Override
	public List<Long> visitStatement(JSStatement statement, long start, long end, int size) {
		return ((JSCronStatement) statement).accept(this, start, end, size);
	}

	@Override
	public List<Long> visitExpression(JSExpression expression) {
		return visitExpression(expression, 0, Long.MAX_VALUE, 0, 0);
	}

	@Override
	public List<Long> visitExpression(JSExpression expression, long start, long end, int size, int months) {
